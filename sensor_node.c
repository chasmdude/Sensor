...
#define LOG_PRINTF(sensorId,temperature,timestamp)							\
      do { 												\
    fprintf(fp_log, "%" PRIu16 " %g %ld\n", (sensorId), (temperature), (long int)(timestamp));	\
    fflush(fp_log);											\
      } while(0)
...
int main(int argc, char *argv[]) {
    ...
    while (i) {
        ...
        time(&data.sensorTs);
        // send data to server in this order (!!): <sensorId><temperature><timestamp>
        ...
        if (tcp_send(client, (void *) &data.sensorTs, &bytes) != TCP_NO_ERROR) exit(EXIT_FAILURE);
        LOG_PRINTF(data.id, data.value, data.sensorTs);
        sleep(sleep_time);
        UPDATE(i);
    }
